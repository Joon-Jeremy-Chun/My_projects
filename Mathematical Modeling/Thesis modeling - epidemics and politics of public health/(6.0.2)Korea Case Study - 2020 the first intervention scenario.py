# -*- coding: utf-8 -*-
"""
Created on Sun Jan 26 22:56:04 2025
...
def load_parameters(file_path):
    ...
    infectious_init = [0, 4.869312685, 5.011367143]  # Define Infectious_init - 2020-02-01
...
real_data = pd.read_csv('DataSets/Korea_threeGroups_SIRV_parameter.csv')
real_data['Date'] = pd.to_datetime(real_data['Date'])
real_data = real_data[(real_data['Date'] >= '2020-02-01') & (real_data['Date'] <= '2020-06-01')]  # Filter between 2020-02-01 and 2020-06-01

time_real = (real_data['Date'] - pd.Timestamp("2020-02-01")).dt.days.values


# Simulation parameters
k = 28  # Days before applying reduction
...
plt.plot(t_combined, I_total_combined, label="Simulated Total Infected", color="blue")
plt.plot(time_real, real_data['I_0-19'] + real_data['I_20-49'] + real_data['I_50-80+'],
         label="Real Total Infected", color="red", linestyle="--")
plt.axvline(x=k, color="green", linestyle="--", label=f"Policy Change at Day {k}")
...
real_columns = ["I_0-19", "I_20-49", "I_50-80+"]
for idx, (label, I) in enumerate(I_groups_combined.items()):
    plt.figure(figsize=(10, 6))
    plt.plot(t_combined, I, label=f"Simulated {label}", color=colors[idx])
    plt.plot(time_real, real_data[real_columns[idx]], label=f"Real {label}", color="red", linestyle="--")
    plt.axvline(x=k, color="green", linestyle="--", label=f"Policy Change at Day {k}")
