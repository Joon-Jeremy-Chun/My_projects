# -*- coding: utf-8 -*-
"""
Created on Thu Sep 18 20:18:08 2024
...
# Adjust starting date for plotting
adjusted_starting_date = pd.to_datetime(starting_date) + pd.to_timedelta(14, unit='days')

# Set initial conditions for the three groups using real data
I_init = [I1_data[0], I2_data[0], I3_data[0]]
initial_conditions = [
...
plt.xlabel('Days (from {} to {})'.format(adjusted_starting_date.strftime('%Y-%m-%d'), ending_date))
plt.ylabel('Infected Population (14-day rolling)')
plt.legend()
plt.title('Parameter Fitting for Transmission Rates ({} to {})'.format(adjusted_starting_date.strftime('%Y-%m-%d'), ending_date))
plt.show()
