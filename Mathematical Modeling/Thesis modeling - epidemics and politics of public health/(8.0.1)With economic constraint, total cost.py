...
W_values = [27.73, 105.12, 92.45]  # Average daily income for each group (in $)
...
def deriv(y, t, N, beta, beta_quarantine, quarantine_day, gamma, mu, W, a):
    ...
    try:
        ...
        dS1dt = -lambda1 * S1 - a[0] * S1 + W_values[0] * R1 + W_values[0] * V1
        dI1dt = lambda1 * S1 - gamma[0] * I1 - mu[0] * I1
        dR1dt = gamma[0] * I1 - W_values[0] * R1 - mu[0] * R1
        dV1dt = a[0] * S1 - W_values[0] * V1
        
        # Group 2 (e.g., adults)
        dS2dt = -lambda2 * S2 + mu[0] * S1 - a[1] * S2 + W_values[1] * R2 + W_values[1] * V2
        dI2dt = lambda2 * S2 + mu[0] * I1 - gamma[1] * I2 - mu[1] * I2
        dR2dt = gamma[1] * I2 + mu[0] * R1 - W_values[1] * R2 - mu[1] * R2
        dV2dt = a[1] * S2 - W_values[1] * V2
        
        # Group 3 (e.g., seniors)
        dS3dt = -lambda3 * S3 + mu[1] * S2 - a[2] * S3 + W_values[2] * R3 + W_values[2] * V3
        dI3dt = lambda3 * S3 + mu[1] * I2 - gamma[2] * I3
        dR3dt = gamma[2] * I3 + mu[1] * R2 - W_values[2] * R3
        dV3dt = a[2] * S3 - W_values[2] * V3
...
def calculate_costs(I, V, D, t):
    ...
    productivity_cost = sum(I[j] * E[j] * W_values[j] for j in range(3))
