...
# Function to read the transmission rates matrix (β_ij) from Excel
def load_transmission_matrix(file_path):
    df = pd.read_excel(file_path, header=None)  # Reading the matrix without header
    return df.values  # Convert dataframe to numpy array

# Load the transmission matrix from an Excel file
file_path = 'transmission_matrix.xlsx'
beta = load_transmission_matrix(file_path)
