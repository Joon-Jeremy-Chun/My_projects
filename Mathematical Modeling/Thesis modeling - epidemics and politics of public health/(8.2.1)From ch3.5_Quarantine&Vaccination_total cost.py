# -*- coding: utf-8 -*-
"""
Created on Tue Mar  4 14:34:59 2025

Author: joonc

Integrated Model with Intervention Metrics and GDP Cost:
  - Uses transmission and dynamic vaccination strategy parameters
  - Incorporates economic cost calculations (medical, vaccination, productivity, funeral)
  - Adds a GDP cost component based on intervention levels:
      Level 1 (β=0.9): 0.5% GDP loss
      Level 2 (β=0.65): 1.0% GDP loss
      Level 3 (β=0.3): 1.5% GDP loss
  - Computes peak infections and peak day for each scenario
  - Scenarios:
      1. Social Distancing Only (dynamic vaccination)
      2. Vaccination Only (manual constant vaccination, no GDP cost)
...
# GDP Parameters
# ---------------------------
GDP_2019 = 1.65e12  # 1.65 trillion USD

def calculate_GDP_cost(beta_multiplier):
    """
    Returns the GDP cost as a fixed cost based on the intervention level.
      - beta_multiplier = 0.9 (Level 1): 0.5% drop
      - beta_multiplier = 0.65 (Level 2): 1.0% drop
      - beta_multiplier = 0.3 (Level 3): 1.5% drop
    """
    if beta_multiplier == 0.9:
        return GDP_2019 * 0.005   # 0.5% loss
    elif beta_multiplier == 0.65:
        return GDP_2019 * 0.01    # 1.0% loss
    elif beta_multiplier == 0.3:
        return GDP_2019 * 0.015   # 1.5% loss
    else:
        return 0.0  # No social distancing → no GDP cost
...
def load_parameters(file_path):
    ...
    # Here "mu" is used as a secondary rate (e.g., maturation or transition)
...
# Load transmission rates matrix from CSV
...
vaccination_rates = np.array([0.0, 0.0, 0.0])  # default; will be overridden in Scenarios 2 & 3
...
# ---------------------------
# For scenarios with social distancing, we add the GDP cost (as a one-time cost).

# Scenario 1: Social Distancing Only (dynamic vaccination)
beta_multiplier_1 = 0.9  # Level 1
beta_mod_1 = beta * beta_multiplier_1
gdp_cost_1 = calculate_GDP_cost(beta_multiplier_1)
t1_sim, sol1, econ_costs1, peak_inf1, peak_day1 = run_simulation(beta_mod_1, use_dynamic=True)
I_total1 = sol1[:, 1] + sol1[:, 5] + sol1[:, 9]
final_cost1 = econ_costs1[-1] + gdp_cost_1
...
plt.hlines(gdp_cost_1, t1_sim[0], t1_sim[-1], colors='blue', linestyles='--', 
           label=f'GDP Cost: ${gdp_cost_1/1e9:.2f}B')
plt.xlabel('Days')
...
print("  Final Simulation Economic Cost: ${:,.2f}".format(econ_costs1[-1]))
print("  GDP Cost Component: ${:,.2f}".format(gdp_cost_1))
print("  Total Economic Cost (Simulation + GDP): ${:,.2f}\n".format(final_cost1))

# Scenario 2: Vaccination Only (manual constant vaccination, no GDP cost)
vaccination_rates = np.array([0.01, 0.01, 0.01])  # constant vaccination rate
beta_mod_2 = beta  # No modification → no social distancing, hence no GDP cost.
t2_sim, sol2, econ_costs2, peak_inf2, peak_day2 = run_simulation(beta_mod_2, use_dynamic=False)
I_total2 = sol2[:, 1] + sol2[:, 5] + sol2[:, 9]
final_cost2 = econ_costs2[-1]  # No additional GDP cost
...
beta_multiplier_3 = 0.9  # Here we again use Level 1 (can be changed to 0.65 or 0.3 for different levels)
beta_mod_3 = beta * beta_multiplier_3
gdp_cost_3 = calculate_GDP_cost(beta_multiplier_3)
t3_sim, sol3, econ_costs3, peak_inf3, peak_day3 = run_simulation(beta_mod_3, use_dynamic=False)
I_total3 = sol3[:, 1] + sol3[:, 5] + sol3[:, 9]
final_cost3 = econ_costs3[-1] + gdp_cost_3
...
plt.hlines(gdp_cost_3, t3_sim[0], t3_sim[-1], colors='blue', linestyles='--', 
           label=f'GDP Cost: ${gdp_cost_3/1e9:.2f}B')
plt.xlabel('Days')
plt.ylabel('Economic Cost ($)')
...
print("  Final Simulation Economic Cost: ${:,.2f}".format(econ_costs3[-1]))
print("  GDP Cost Component: ${:,.2f}".format(gdp_cost_3))
print("  Total Economic Cost (Simulation + GDP): ${:,.2f}".format(final_cost3))
