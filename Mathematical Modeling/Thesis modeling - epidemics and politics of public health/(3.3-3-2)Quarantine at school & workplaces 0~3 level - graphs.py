...
# Global quarantine levels (applied as multipliers to baseline beta entries)
# For example, a level of 0.3 means a 30% reduction yielding a multiplier of 0.7.
...
# For the combined school and workplace scenario, adjust both beta[0,0] and beta[1,1]
def compute_peak_reduction_and_delay_all_combined(quarantine_levels, beta, N, gamma, mu, W, a, initial_conditions, time_span):
    ...
    # Apply combined school and workplace quarantine by modifying beta[0,0] and beta[1,1]
    for level in quarantine_levels[1:]:
        adjusted_beta = beta.copy()
        adjusted_beta[0, 0] *= (1 - level)  # School-to-school transmission reduced
        adjusted_beta[1, 1] *= (1 - level)  # Workplace-to-workplace transmission reduced
...
# Compute combined scenario peak reduction and delay using global quarantine_levels
peak_results_combined = compute_peak_reduction_and_delay_all_combined(quarantine_levels, beta, N, gamma, mu, W, a, initial_conditions, time_span)

# Print combined scenario results
for group, results in peak_results_combined.items():
    print(f"Combined Scenario Results for {group}:")
...
# Plotting functions for combined school and workplace scenario

def plot_group_infected_4_levels_combined(group_idx, group_label, beta, N, gamma, mu, W, a, initial_conditions, time_span):
    t = np.linspace(0, time_span, int(time_span))
    ...
    for i, level in enumerate(quarantine_levels):
        adjusted_beta = beta.copy()
        # Modify both school and workplace transmission rates
    ...
    plt.title(f'Infectious Population for {group_label} under Combined School & Workplace Quarantine')
    plt.xlabel('Days')
    plt.ylabel(f'Infectious Population ({group_label})')
    plt.legend(loc='upper right')
    plt.tight_layout()
    
    # Save the figure with filename starting with "school_workplace_quarantine_"
    filename = f"Figures/school_workplace_quarantine_{group_label.replace(' ', '_')}_Infected.png"
    plt.savefig(filename)
    plt.show()

def plot_total_infected_4_levels_combined(beta, N, gamma, mu, W, a, initial_conditions, time_span):
    t = np.linspace(0, time_span, int(time_span))
    plt.figure(figsize=(10, 6))
    colors = ['blue', 'green', 'orange', 'red']
    ...
    plt.title('Total Infectious Population under Combined School & Workplace Quarantine')
    plt.xlabel('Days')
    ...
    # Save the figure with filename starting with "school_workplace_quarantine_"
    plt.savefig("Figures/school_workplace_quarantine_Total_Infected_Population.png")
    plt.show()

# Plot for each group under combined scenario
plot_group_infected_4_levels_combined(0, "Group 1 (Children)", beta.copy(), N, gamma, mu, W, a, initial_conditions, time_span)
plot_group_infected_4_levels_combined(4, "Group 2 (Adults)", beta.copy(), N, gamma, mu, W, a, initial_conditions, time_span)
plot_group_infected_4_levels_combined(8, "Group 3 (Seniors)", beta.copy(), N, gamma, mu, W, a, initial_conditions, time_span)

# Plot the total infected population under combined scenario
plot_total_infected_4_levels_combined(beta.copy(), N, gamma, mu, W, a, initial_conditions, time_span)
