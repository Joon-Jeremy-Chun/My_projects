# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 09:20:49 2024
...
# Objective function to fit parameters
def objective(params, t, N, initial_conditions, I_data, S_data, gamma, mu, W):
    # Update the transmission rates, recovery rates, and waning immunity rate based on input params
    beta_flat = params[:9]
    gamma = params[9:12]
    W = params[12]
    a = params[13:16]
    ...
    S1, I1, _, _, S2, I2, _, _, S3, I3, _, _ = results.T
    
    # Calculate the sum of squared errors between model and observed data
    error = (
        np.sum((I1 - I_data[0])**2) + np.sum((I2 - I_data[1])**2) + np.sum((I3 - I_data[2])**2) +
        np.sum((S1 - S_data[0])**2) + np.sum((S2 - S_data[1])**2) + np.sum((S3 - S_data[2])**2)
    )
    return error
...
ending_date = "2022-08-01"
(beta, gamma, mu, W, a, time_span, N, S_init, I_init, R_init, V_init, beta_quarantine, quarantine_day) = load_parameters(inputs_path)
observed_data = load_observed_data(data_path, starting_date, ending_date)

# Extract observed infected and susceptible data for each group
I1_data = observed_data[observed_data['new_age_group'] == '0-19']['new_confirmed_cases'].values
I2_data = observed_data[observed_data['new_age_group'] == '20-49']['new_confirmed_cases'].values
I3_data = observed_data[observed_data['new_age_group'] == '50-80+']['new_confirmed_cases'].values
I_data = [I1_data, I2_data, I3_data]

S1_data = observed_data[observed_data['new_age_group'] == '0-19']['confirmed_cases'].values
S2_data = observed_data[observed_data['new_age_group'] == '20-49']['confirmed_cases'].values
S3_data = observed_data[observed_data['new_age_group'] == '50-80+']['confirmed_cases'].values
S_data = [S1_data, S2_data, S3_data]
...
initial_guess = np.concatenate((beta.flatten(), gamma, [W], a))

# Optimize parameters
result = minimize(objective, initial_guess, args=(t, N, initial_conditions, I_data, S_data, gamma, mu, W))

# Extract fitted parameters
fitted_params = result.x
fitted_beta = fitted_params[:9].reshape((3, 3))
fitted_gamma = fitted_params[9:12]
fitted_W = fitted_params[12]
fitted_a = fitted_params[13:16]

print("Fitted Transmission Rates:")
print(fitted_beta)
print("Fitted Recovery Rates:")
print(fitted_gamma)
print("Fitted Waning Immunity Rate:")
print(fitted_W)
print("Fitted Vaccination Rates:")
print(fitted_a)

# Plot results
results = odeint(deriv, initial_conditions, t, args=(N, fitted_beta, fitted_gamma, mu, fitted_W, fitted_a))
_, I1, _, _, _, I2, _, _, _, I3, _, _ = results.T

plt.figure(figsize=(10, 6))
plt.plot(t, I1_data, 'ro', label='Observed Infected (0-19)')
plt.plot(t, I1, 'r-', label='Fitted Infected (0-19)')
plt.plot(t, I2_data, 'bo', label='Observed Infected (20-49)')
plt.plot(t, I2, 'b-', label='Fitted Infected (20-49)')
plt.plot(t, I3_data, 'go', label='Observed Infected (50-80+)')
plt.plot(t, I3, 'g-', label='Fitted Infected (50-80+)')
plt.xlabel('Days')
plt.ylabel('Infected Population')
plt.legend()
plt.title('Parameter Fitting for Transmission Rates, Recovery Rates, Waning Immunity, and Vaccination Rates')
plt.show()
